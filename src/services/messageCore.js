...
const sendMessageCore = async (message, tableName, bucketName) => {
  const { type, content, file, fileName, mimeType, metadata, isAnonymous, isSecret, quality, replyToMessageId } = message;

  console.log('Core - type:', type);
  console.log('Core - content:', content);

  // Danh sách loại tin nhắn hợp lệ
  const validTypes = ['text', 'image', 'file', 'video', 'voice', 'sticker', 'location', 'contact', 'poll', 'event'];
  if (!validTypes.includes(type)) {
    throw new Error('Loại tin nhắn không hợp lệ!');
  }

  // Kiểm tra đầu vào theo loại tin nhắn
  if (type === 'text' && (!content || typeof content !== 'string' || content.trim() === '')) {
    throw new Error('Nội dung văn bản không hợp lệ!');
  }
  if (['image', 'file', 'video', 'voice', 'sticker'].includes(type) && (!file || !Buffer.isBuffer(file) || !mimeType)) {
    throw new Error('File hoặc MIME type không hợp lệ!');
  }
  if (type === 'location' && (!metadata || !metadata.latitude || !metadata.longitude)) {
    throw new Error('Vị trí cần có latitude và longitude!');
  }
  if (type === 'contact' && (!metadata || !metadata.name || !metadata.phone)) {
    throw new Error('Danh bạ cần có tên và số điện thoại!');
  }
  if (type === 'poll' && (!metadata || !metadata.question || !Array.isArray(metadata.options))) {
    throw new Error('Khảo sát cần có câu hỏi và danh sách tùy chọn!');
  }
  if (type === 'event' && (!metadata || !metadata.title || !metadata.date)) {
  ...
  const mimeTypeMap = {
    ...
    'image/png': { type: 'image', folder: 'images', ext: 'png', maxSize: 10 * 1024 * 1024 },
    'image/heic': { type: 'image', folder: 'images', ext: 'heic', maxSize: 10 * 1024 * 1024 },
    'image/gif': { type: 'image', folder: 'images', ext: 'gif', maxSize: 10 * 1024 * 1024 },
  ...
  if (['image', 'file', 'video', 'voice', 'sticker'].includes(type)) {
    ...
    if (!mimeInfo || mimeInfo.type !== type) {
